...
master push  → install → lint → typecheck → test → OpenNext build → deploy → legacy redirect → smoke test
```
...
- 메인 Worker 배포 뒤 기존 `investlens` Worker를 새 Impacticker 주소로 보내는 301 리디렉션 Worker를 배포합니다.
- 같은 Pull Request의 이전 실행은 취소하지만 운영 배포는 중간에 취소하지 않습니다.
- 운영 확인은 캐시를 우회해 공개 랜딩 `/`이 정상 Impacticker HTML을 반환할 때까지 제한적으로 재시도합니다.
- 기존 주소의 경로와 쿼리 문자열이 새 주소에 그대로 보존되는지도 별도 스모크 테스트로 검증합니다.
- Dependabot이 npm 패키지와 GitHub Actions 업데이트를 매주 묶어서 제안합니다.

워크플로 파일은 [`.github/workflows/ci-deploy.yml`](../.github/workflows/ci-deploy.yml), 운영 스모크 테스트는 [`scripts/smoke-deployment.mjs`](../scripts/smoke-deployment.mjs), 기존 주소 전환 검증은 [`scripts/smoke-legacy-redirect.mjs`](../scripts/smoke-legacy-redirect.mjs)에 있습니다.
...
| `CLOUDFLARE_API_TOKEN` | `impacticker`, `investlens` Worker 배포 권한 |
