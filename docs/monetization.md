...
| `0.2.1` | 전용 광고 단위·운영 변수 구성, AdSense 사이트 승인 준비 | 운영 준비 |
...
- Impacticker 랜딩과 대시보드 전용 반응형 광고 단위를 생성했습니다.
- 게시자 ID, 두 슬롯 ID, 개인정보 문의 주소를 GitHub Actions Variables에 등록했습니다.
- 운영 배포에서 `/ads.txt`와 개인정보 문의 주소를 제공할 수 있습니다.
- 광고 활성화 값은 사이트 승인 전까지 `false`로 유지합니다.
- `workers.dev` 주소는 AdSense가 Worker 이름보다 상위의 계정 도메인을 요구하므로 현재 운영 URL을 직접 사이트로 등록할 수 없습니다.
- 커스텀 최상위 도메인을 연결한 뒤 AdSense 사이트 검토와 CMP 게시를 완료해야 실제 광고를 활성화할 수 있습니다.

도메인을 연결하기 전에는 광고 슬롯이 화면에 나타나지 않으며 광고 요청도 전송되지 않습니다. 도메인 구매나 이전은 비용과 소유권 결정이 필요한 별도 작업으로 취급합니다.
