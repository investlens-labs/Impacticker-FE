...
describe('InstrumentLogo', () => {
  ...
  it('이미지 로딩에 실패하면 종목명 첫 글자를 표시한다', () => {
    renderWithIntl(<InstrumentLogo companyName="Apple Inc." logoUrl="https://example.com/missing.png" />, { locale: 'ko' })
  ...
  it('로고 URL이 없으면 티커를 fallback으로 사용한다', () => {
    renderWithIntl(<InstrumentLogo companyName="" ticker="qqq" logoUrl={null} size={48} />, { locale: 'ko' })
...
describe('LogoAttribution', () => {
  it('API 응답의 출처 URL을 새 탭 링크로 표시한다', () => {
    renderWithIntl(<LogoAttribution url="https://logo.dev" />, { locale: 'ko' })

    expect(screen.getByRole('link', { name: '로고 제공: Logo.dev' })).toHaveAttribute('href', 'https://logo.dev')
    expect(screen.getByRole('link', { name: '로고 제공: Logo.dev' })).toHaveAttribute('target', '_blank')
  })
})
