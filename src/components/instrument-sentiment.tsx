import { BarChart3, Minus, Sparkles, TrendingDown, TrendingUp } from 'lucide-react'
...
const sentimentItems = [
  {
    ...
    icon: TrendingUp,
    barClass: 'bg-red-500',
    surfaceClass: 'border-red-100 bg-red-50/70 dark:border-red-950 dark:bg-red-950/25',
  ...
  },
  {
    ...
    surfaceClass: 'border-blue-100 bg-blue-50/70 dark:border-blue-950 dark:bg-blue-950/25',
  ...
  },
  {
    ...
    surfaceClass: 'border-slate-200 bg-slate-50/80 dark:border-slate-700 dark:bg-slate-800/70',
    iconClass: 'bg-slate-200 text-slate-600 dark:bg-slate-700 dark:text-slate-200',
...
export function InstrumentSentiment({ sentiment }: { sentiment: InstrumentNewsSentiment }) {
  ...
  return (
    <section className="mb-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900" aria-labelledby="instrument-sentiment-title">
      ...
      <div className="mt-4 flex h-3 w-full gap-px overflow-hidden rounded-full bg-slate-100 ring-1 ring-inset ring-slate-200 dark:bg-slate-800 dark:ring-slate-700" role="img" aria-label={t('aria', { up: sentiment.upPercentage, down: sentiment.downPercentage, neutral: sentiment.neutralPercentage })}>
      ...
      <dl className="mt-3 grid grid-cols-3 gap-2">
        {sentimentItems.map((item) => (
          <div key={item.key} className={`min-w-0 rounded-lg border px-2.5 py-2.5 sm:px-3 ${item.surfaceClass}`}>
            <dt className="flex items-center gap-1.5 text-xs font-medium text-slate-600 dark:text-slate-300">
              <span className={`grid size-5 shrink-0 place-items-center rounded-md ${item.iconClass}`} aria-hidden>
                <item.icon className="size-3.5" />
              </span>
              <span className="truncate">{newsT(item.labelKey)}</span>
            </dt>
            <dd className={`mt-1.5 font-mono text-lg font-bold leading-none tabular-nums ${item.textClass}`}>
              {sentiment[item.key]}%
            </dd>
          </div>
        ))}
